...
memtier_benchmark \
  ...
  --rate-limiting "${MEMTIER_POSITIONS_RATE_PER_CONNECTION:-188}" \
  --test-time "${MEMTIER_TEST_TIME:-60}" \
  --command "__monitor_line@__" \
  --monitor-input monitor-input/positions-by-account.txt \
  --monitor-pattern R \
  --json-out-file memtier-output/positions-by-account.json \
